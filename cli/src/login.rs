...
use ruda::api::{self, AuthDuration, AuthResponse, AuthScope};
...
pub fn cmd() -> clap::Command {
    ...
    Command::new("login")
        .about("Log in with ruda.app credentials")
        .long_about(
...
/// Authenticates user with dash credentials and stores the resulting
/// application token on the filesystem.
pub async fn run(matches: &ArgMatches, cancellation: CancellationToken) -> Result<()> {
    ...
    if let Some((email, password)) = creds {
        ...
        let response = reqwest::Client::new()
            // .post("https://ruda.app/api/auth")
            .post("http://127.0.0.1:10000/api/auth")
            .json(&request)
            .send()
            .await?;

        if response.status() == StatusCode::from_u16(500).unwrap() {
